...
const getLoginFollowList = async () => {
  const res = await fanGetFollowService(userStore.user.id, 'follow')
...
const getFollowList = async () => {
  ...
  const fan1Ids = followList.value.map((item) => item.fan.id)
...
<template>
  ...
  >
    ...
    >
      <span @click="toUserInfo(follow.fan.id)" class="cover">
        <el-avatar class="image" :src="follow.fan.avatar"></el-avatar>
      </span>
      <span @click="toUserInfo(follow.fan.id)" class="name">{{
        follow.fan.name
      }}</span>
      <span class="button">
        <el-button
          type="info"
          @click="onFan(follow.fan)"
          round
          v-if="equalsFans.includes(follow.fan.id)"
        >
          已关注
        </el-button>
        <el-button type="primary" round @click="onFan(follow.fan)" v-else>
          +关注
        </el-button>
      </span>
    </li>
